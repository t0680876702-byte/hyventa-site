    ...
    <div class="cfg-agent">
      ...
      <div class="cfg-agent-body">
        <div class="cfg-agent-head">
          ...
          <span class="cfg-agent-role">${esc(a.role)}</span>
        </div>
        <p class="cfg-agent-blurb">${esc(AGENT_BLURB[a.id] || '')}</p>
      </div>
    </div>`).join('');
...
function updateMatchHint(root) {
  ...
  box.innerHTML = matched.length === 1
    ? `<span class="cfg-match-kicker">${esc(t('match_one'))}</span><strong>${esc(matched[0].name)}</strong> — ${esc(matched[0].role)}`
